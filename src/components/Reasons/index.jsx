...
const Reasons = () => {
  return (
    ...
    <div className='container mx-auto  flex flex-col  gap-6 items-start px-5 py-16 lg:py-20'>
      {/* Heading */}
      <h2 className="text-[70px] lg:text-[96px] font-bold text-green uppercase pb-4 leading-none">
        5 REASONS 
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
        ...
        <div className="space-y-6">
          {reasons.map((reason, index) => (
              ...
              <span className="text-3xl lg:text-4xl font-medium  mr-3 lg:mr-16">
                {String(index + 1).padStart(2, "0")}.
              </span>
              <div>
                <h3 className="text-3xl lg:text-4xl font-medium  ">{reason.title}</h3>
                <p className="text-gray-600 mt-7">{reason.description}</p>
              </div>
            </div>
          ))}
        </div>
         ...
         <div className='h-[200px] lg:h-[249px] flex justify-center items-center px-5 bg-green text-white'>
            <p className='text-3xl md:text-4xl lg:text-5xl'>The show is completely <span className='font-bold'>indoors </span>so don't let the weather stop you.</p>
